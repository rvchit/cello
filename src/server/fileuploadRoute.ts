...
router.post("/upload-part", multerUpload.single("fileChunk"), async (req: Request, res: Response) => {
  ...
  const { fileName, partNumber, uploadId }: { fileName: string; partNumber: number; uploadId: string } = req.body;
...
router.post("/complete-upload", async (req: Request, res: Response) => {
  const { fileName, uploadId, parts }: { fileName: string; uploadId: string; parts: Array<{ ETag: string; PartNumber: number }> } = req.body;
